...
public class CustomeClassLoader extends ClassLoader {
    ...
    public Class<?> loadClass(String name) throws ClassNotFoundException{
        ...
        if(!exceedThreshold()){
            ...
            LOG.error("通过 com.shzz.common.tool.bean.transform.asm.TransformUtilGenerate 创建的类所占用内存空间 {},超过阈值50*2014*1024字节",staticSize()/(1024*1024));
            throw new ClassNotFoundException("通过TransformUtilGenerate 创建的类所占用总内存空间超过阈值50*1024*1024字节，新建类无法创建, 可修改系统配置 "+SystemProperties.CLASS_SIZE_THRESHOLD+" 值 (单位 byte)");
        }
